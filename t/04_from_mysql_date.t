...
subtest "as gmtime" => sub {
    my $parsed = Time::Piece->from_mysql_date(str => $sometime, as_localtime => 0);
...
subtest "as localtime" => sub {
    my $parsed = Time::Piece->from_mysql_date(str => $sometime, as_localtime => 1);
    isa_ok($parsed => 'Time::Piece', "returns Time::Piece instance");
...
subtest "epoch minus date" => sub {
    ...
    my $parsed = Time::Piece->from_mysql_date(str => $somoday, as_localtime => 0);
