...
subtest "as gmtime" => sub {
    my $parsed = Time::Piece->parse_mysql_datetime(str => $sometime, as_localtime => 0);
...
subtest "as localtime" => sub {
    my $parsed = Time::Piece->parse_mysql_datetime(str => $sometime, as_localtime => 1);
    isa_ok($parsed => 'Time::Piece', "returns Time::Piece instance");
...
subtest "epoch minus datetime" => sub {
    ...
    my $parsed = Time::Piece->parse_mysql_datetime(str => $somoday, as_localtime => 0);
